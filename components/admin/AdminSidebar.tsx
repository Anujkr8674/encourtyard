...
export const AdminSidebar: React.FC<{ onCloseMobile?: () => void }> = ({ onCloseMobile }) => {
  ...
  return (
    <aside className="w-64 sm:w-72 bg-[#F6F4ED] text-[#181F18] border-r border-[#E0DCD3] flex flex-col justify-between h-full select-none shadow-xl relative overflow-hidden">
          ...
          {ADMIN_NAV_ITEMS.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                onClick={onCloseMobile}
                className={`w-full px-3.5 py-2.5 rounded-xl text-xs font-semibold flex items-center justify-between transition-all duration-200 group cursor-pointer ${
                  isActive
              ...
              >
                <div className="flex items-center gap-3">
                  <Icon
                    className={`w-4.5 h-4.5 transition-all duration-200 shrink-0 ${
                      isActive
                ...
                <div className="flex items-center gap-1.5">
                  {item.badge && (
                    <span
                      className={`px-2 py-0.5 rounded-full text-[10px] font-bold font-mono transition-transform duration-200 group-hover:scale-105 ${
                        item.badgeColor || (isActive ? 'bg-white text-[#1E5C23] shadow-sm' : 'bg-emerald-100 text-[#1E5C23] border border-emerald-200')
                      }`}
                    >
                      {item.badge}
                    </span>
      ...
      <div className="p-3.5 border-t border-[#E0DCD3] space-y-2 bg-[#EEEBE2]">
        <Link
          href="/"
          target="_blank"
